...
If you haven't heard of ANNOY - it's a method for finding k nearest neighbours using binary search 0(log n).   
<br>
<div class="row">
...
<br>
As an experiment I created a basic transformer attention network (like ViViT) but used pre-trained convolutional networks to encode scenes from movie trailers rather than mapping attention at a pixel level (I only have a 1080TI!)
I used an I3D Kinetics network to get a temporal feature for every 16 frames and a ResNet to get spatial data for every scene.   
<br>
These are passed through the transformer encoder and then a linear layer 
combines the spatial and temporal output features to create an embedding vecor. I then added a projection head for classification which will be removed after training.  
<br>
...
To better understand the results I then collected the embedding vectors of the test set and setup this streamlit app to do super fast lookup with ANNOY.   
...
<div class="caption">
    Using just the visual features results in pretty accurate recommendations.
</div>
<br>
